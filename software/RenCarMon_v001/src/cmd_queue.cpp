...
{
	if(!cmd_ctr) return 0;
	cmd_t *nw_cmd=(cmd_t *)calloc(1, sizeof(cmd_t));
	uint8_t *nw_data=(uint8_t*)calloc(data_len, sizeof(uint8_t));
	if(!nw_cmd || !nw_data) return 0;	// Couldnt allocate memory

	nw_cmd->cmd=cmd;
	nw_cmd->data_len=data_len;
	nw_cmd->data=nw_data;
	memcpy(nw_cmd->data, data, data_len);

	if(queue)queue->next=nw_cmd;
	else queue=nw_cmd;				// Else if new command is first command receive by the system
